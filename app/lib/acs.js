...
exports.registerUser = function (params,callback){
		...
		var index = params.username.indexOf('@');
		var firstName = (params.username.value).substring(0,index);
		 Cloud.Users.create({
            username:params.username,
            email:params.username,
            password:params.pwd,
            password_confirmation:params.pwd
           // custom_fields:{"coordinates":[lng,lat] },
           // last_name:,
            
        }, function (e) {
        	if (e.success) {
	        	var user  = e.users[0];
		        APP.user.uname = user.username;
		        Ti.App.Properties.setString('uname', user.username);
		        APP.user.id = user.id;
		        Ti.API.info('after Login in success APP.user.id = '+ APP.user.id);
		        Ti.App.Properties.setString('uid',user.id);
		        APP.user.sessionID = e.meta.session_id;
...
exports.getPosts = function(radius,_lon,_lat,callback){
	var maxDistance = radius/3959;
	Cloud.Posts.query({
		order: '-created_at',
		per_page: 10,
